...
//		Log.i(TAG, "start alarm ping sender to MqttService"+ action);
//
//        this.mCallbackThread = new HandlerThread("IM_MQTT_PING_THREAD", Process.THREAD_PRIORITY_FOREGROUND);
//        this.mCallbackThread.start();
//        mHandler = new Handler( this.mCallbackThread.getLooper()) {
...
//                        IMqttToken token = that.comms.checkForActivity(new IMqttActionListener() {
//
//                            @Override
...
//	public void stop() {
//
//		Log.i(TAG, "stop alarm ping sender to MqttService"+comms.getClient().getClientId());
//		if(hasStarted){
...
//            bundle.putLong("time", delayInMilliseconds);
...
class AlarmPingSender implements MqttPingSender {
	...
	public void schedule(long delayInMilliseconds) {
        ...
        if(Build.VERSION.SDK_INT >= 21){
			...
			// the device to run this task whilst dosing.
			Log.d(TAG, "Alarm scheule using setExactAndAllowWhileIdle, next: " + delayInMilliseconds);
			AlarmManager.AlarmClockInfo info =  new AlarmManager.AlarmClockInfo(nextAlarmInMilliseconds, pendingIntent);
